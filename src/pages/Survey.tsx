...
const SurveyPage: React.FC = () => {
  ...
  const [userEmail, setUserEmail] = useState('survey@mentorino.me');

  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        setUserEmail(session.user.email || 'survey@mentorino.me');
        setUserName(session.user.user_metadata?.full_name || session.user.email?.split('@')[0] || 'Anonymous');
      }
    });
  }, []);
  ...
  const handleSubmit = async () => {
    if (!rating) return;
    setSubmitting(true);
    const res = await fetch('/api/emails?from=review', {
      method: 'POST',
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 sm:py-12 px-4 animate-in fade-in duration-700">
      <SEO 
        title="Session Audit" 
        description="Share your session feedback with Mentorino. Rate your experience and help us improve your guidance journey."
      />
      <button 
        onClick={() => navigate(-1)}
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map(i => (
                    <button key={i} onClick={() => setRating(i)} className={`w-10 h-10 sm:w-12 sm:h-12 ${rating === i ? 'bg-black text-white' : 'bg-slate-50'} rounded-xl sm:rounded-2xl font-black text-xs hover:bg-black hover:text-white transition-all shadow-sm active:scale-90`}>{i}</button>
                  ))}
                </div>
              </div>
              <div className="space-y-2">
                ...
                <textarea value={comment} onChange={(e) => setComment(e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-medium outline-none focus:border-black transition-all min-h-[120px]" placeholder="Key insights..." />
              </div>
              <button onClick={handleSubmit} disabled={submitting || !rating} className="btn-normal w-full py-4 sm:py-5 bg-black text-white flex items-center justify-center gap-2">{submitting && <Loader2 size={16} className="animate-spin" />}Submit Evaluation</button>
            </div>
          </>
        )}
      </div>
