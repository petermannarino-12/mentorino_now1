...
import { useAddBookingMutation } from '../src/hooks/useBookings';
...
const BookingPage: React.FC<BookingPageProps> = () => {
  ...
  const isSunday = (day: number) => new Date(currentYear, currentMonth - 1, day).getDay() === 0;
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 sm:py-8 lg:py-12 px-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <SEO 
        ...
        description="Schedule a strategic mentorship session with Mentorino. Pick a date and time slot that works for your growth journey."
      />
      <button 
        onClick={() => navigate(-1)}
      ...
      <div className="bg-white rounded-[32px] sm:rounded-[48px] border border-black/[0.03] shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="p-6 sm:p-10 md:p-12 border-b lg:border-b-0 lg:border-r border-black/[0.02]">
          <div className="flex items-center justify-between mb-8 sm:mb-10">
            <h3 className="text-[11px] sm:text-sm font-black uppercase tracking-widest text-slate-900">{MONTHS[currentMonth - 1]} {currentYear}</h3>
            <div className="flex gap-1 sm:gap-2">
              <button onClick={prevMonth} className="p-1.5 sm:p-2 hover:bg-slate-50 rounded-xl text-slate-400 transition-colors"><ChevronLeft size={16} className="sm:w-[18px] sm:h-[18px]" /></button>
              <button onClick={nextMonth} className="p-1.5 sm:p-2 hover:bg-slate-50 rounded-xl text-slate-400 transition-colors"><ChevronRight size={16} className="sm:w-[18px] sm:h-[18px]" /></button>
            </div>
          </div>
            ...
            {days.map(day => (
              <button 
                ...
                className={`
                  ...
                  ${isSunday(day) ? 'opacity-10 pointer-events-none' : ''}
                `}
              >
                {day}
              </button>
        ...
        <div className="p-6 sm:p-10 md:p-12 bg-slate-50/50 flex flex-col">
          <div className="flex items-center gap-3 sm:gap-4 mb-8 sm:mb-10">
            ...
            <div>
              ...
              <div className="text-[11px] sm:text-sm font-black uppercase">{MONTHS[currentMonth - 1]} {selectedDate}, {currentYear}</div>
            </div>
          </div>
          ...
          <button 
            ...
            `}
          >
            {isBooking ? <Loader2 size={16} className="animate-spin" /> : 'Confirm Session'}
          </button>
